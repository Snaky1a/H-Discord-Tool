...
def from_datetime_to_humanly(
    date: datetime, to_string: Union[datetime.strptime, str] = "%d.%m.%Y %H:%M:%S"
) -> str:
    """
...
def from_iso_format_to_humanly(
    iso: str, to_string: Union[datetime.strptime, str] = "%d.%m.%Y %H:%M:%S"
) -> str:
    """
...
async def check_nitro_credit(headers: Dict[Any, Any]) -> Tuple[int, int]:
    ...
    async with aiohttp.ClientSession(
        base_url=BASE_URL, timeout=aiohttp.ClientTimeout(total=10)
    ) as session:
...
async def check_payments(headers: Dict[Any, Any]) -> Optional[Union[List[str], int]]:
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get(
            "/api/v10/users/@me/billing/payment-sources", headers=headers
        ) as response:
            if response.status == 200:
...
async def get_guilds(headers: Dict[Any, Any]) -> Dict[str, List[str]]:
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get(
            "/api/v9/users/@me/guilds?with_counts=true", headers=headers
        ) as response:
            if response.status == 200:
...
async def get_gifts(headers: Dict[Any, Any]) -> List[str]:
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get(
            "/api/v10/users/@me/entitlements/gifts", headers=headers
        ) as response:
            if response.status == 200:
                r_json = await response.json()
...
async def get_connections(headers: Dict[Any, Any]) -> Dict[Any, Any]:
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get(
            "/api/v10/users/@me/connections", headers=headers
        ) as response:
            if response.status == 200:
...
async def get_promotions(
    headers: Dict[Any, Any], locale: Optional[str] = None
) -> Dict[Any, Any]:
    """
...
async def check_boosts(headers: Dict[Any, Any]) -> Dict[Any, Any]:
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get(
            "/api/v10/users/@me/guilds/premium/subscription-slots", headers=headers
        ) as response:
            if response.status == 200:
                info = await response.json()
...
async def get_nitro_info(
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get(
            "/api/v10/users/@me/billing/subscriptions", headers=headers
        ) as resp:
            if resp.status == 200:
                nitro_billing = await resp.json()

    if nitro_billing:
        nitro_start = from_iso_format_to_humanly(
...
async def get_relationships(headers: Dict[Any, Any]) -> Tuple[int, List[str]]:
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get(
            "/api/v10/users/@me/relationships", headers=headers
        ) as resp:
            relationship_json = await resp.json()

    if (
        relationship_json
        and not isinstance(relationship_json, list)
...
async def check_token(token: str, mask_token: bool = True) -> None:
    ...
    if status == 200:
        ...
        nitro_credits = (
            f"Nitro classic/boost credits: {classic_credits}/{nitro_boost_credits}"
        )
        ...
        avatar_format = "gif" if str(info.get("avatar", "")).startswith("a_") else "png"
        banner_format = "gif" if str(info.get("banner", "")).startswith("a_") else "png"
        avatar = (
            f"https://cdn.discordapp.com/avatars/{info.get('id')}/{info.get('avatar')}."
            f"{avatar_format}"
            if info.get("avatar")
            else None
        )
        banner = (
            f"https://cdn.discordapp.com/banners/{info.get('id')}/{info.get('banner')}."
            f"{banner_format}"
            if info.get("banner")
            else None
        )
